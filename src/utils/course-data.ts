...
export const getFirstLesson = () => {
  const firstChapterWithLessons = courseData.find(chapter => chapter.lessons.length > 0);
  if (!firstChapterWithLessons) {
    throw new Error('No lessons found in course data');
  }
  return {
    lesson: firstChapterWithLessons.lessons[0],
    chapter: firstChapterWithLessons
  };
};
  ...
  for (let i = 0; i < courseData.length; i++) {
    ...
    for (let j = 0; j < chapter.lessons.length; j++) {
      ...
      if (lesson.slug === currentSlug) {
        if (j > 0) {
          // Previous lesson in the same chapter
          return { lesson: chapter.lessons[j - 1], chapter };
        } else if (i > 0) {
          // Last lesson of the previous chapter
          const previousChapter = courseData[i - 1];
          const lastLessonIndex = previousChapter.lessons.length - 1;
          return { 
            lesson: previousChapter.lessons[lastLessonIndex],
